...
from typing import Dict, cast, Any, List, Tuple
...
def _normalize_string_type(value: Any) -> Any:
...
def compare_schemas(one: JSONSchemaProperty, two: JSONSchemaProperty) -> bool:
    ...
    if isinstance(one, list):
        ...
        return _compare_dicts(cast(dict, one), cast(dict, two))
...
def convert_python_regex_to_ecma(value: str, flags: List[re.RegexFlag]=[]) -> str:
    ...
    result_flags_str = ''.join(result_flags)

    return '/{value}/{flags}'.format(value=value, flags=result_flags_str)
